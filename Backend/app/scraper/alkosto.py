...
class AlkostoScraper(BaseScraper):
    ...
    async def search(self, query: str) -> list[ScrapedPrice]:
        ...
        try:
            ...
            await page.wait_for_selector("article.product__item", timeout=12000)

            items = await page.query_selector_all("article.product__item")

            for item in items[:5]:
                try:
                    title_el = await item.query_selector("h3.product__item__top__title")
                    price_el = await item.query_selector("strong.product__item__top__price--special") \
                               or await item.query_selector("span.js-price-selector")
                    link_el  = await item.query_selector("a.product__item__top__title--link")
